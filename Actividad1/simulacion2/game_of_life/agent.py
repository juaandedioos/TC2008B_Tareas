...
class Cell(FixedAgent):
    ...
    def upper_neighbors(self):
        ...
        left = center = right = 0 # Valores iniciales
        H = self.model.grid.height 
        W = self.model.grid.width

        uy = (self.y + 1) % H # Coordenada y del vecino superior
        lx = (self.x - 1) % W # Coordenada x del vecino izquierdo
        cx = self.x % W       # Coordenada x del vecino central
        rx = (self.x + 1) % W # Coordenada x del vecino derecho

        for n in self.neighbors: 
            if n.y == uy: 
                if n.x == lx:
                    left = 1 if n.is_alive else 0 
                elif n.x == cx:
                    center = 1 if n.is_alive else 0
                elif n.x == rx:
                    right = 1 if n.is_alive else 0
